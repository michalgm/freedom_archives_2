...
SELECT
    ...
    (
        SELECT
            ARRAY_AGG(
                DISTINCT (VALUE.value->>'collection_id'::TEXT)::INTEGER
            ) AS ARRAY_AGG
        FROM
            UNNEST(c.children) VALUE (VALUE)
    ) AS collection_ids,
     featured_records,
    ARRAY(
        SELECT row_to_json(c3)
        FROM (
        SELECT collection_id, collection_name, thumbnail, description, summary
        FROM collections c2
