...
import { setUser, updateListItemRelations, prepListItemRelations, refreshView, writeThumbnailsFromUrl, fetchUnified, setArchive } from "../common_hooks/index.js";
const tsquery = pgTsquery();
const fullTextSearch = (context) => {
  const { service: { fullName }, } = context;
...
const lookupFilters = async ({ params: { knex, query: { $fullText },
// transaction: { trx },
  ...
  if ($fullText !== undefined) {
    ...
    ].flatMap(async (query) => (await Model.raw(query)).rows.flat()))).flat();
...
const prepData = (context) => {
  ...
  if (data && Object.keys(data).length) {
    ...
    Object.keys(data).forEach((key) => {
      if (["call_numbers", "formats", "qualitys", "generations", "media_types", "siblings", "relationships"].includes(key) ||
                key.match("_search")) {
        delete data[key];
      }
    });
...
const updateRelations = async (context) => {
  ...
  if (relation_data.instances !== undefined) {
    const instances = await Promise.all(relation_data.instances.map((instance) => {
      if (instance.delete) {
