...
import { isArray, startCase, update } from "lodash-es";
...
const transformData = (data) => {
  data.instances.forEach((i) => (i.delete = false));
  // logger.log(data.instances);
  return data;
};
...
function Record({ showForm, id, ro = false /*  embedded = false */ }) {
  ...
  const loadRecord = useCallback(async (record_data, relationships) => {
    ...
    // logger.log(record.primary_instance_id)
    setRecord(record);
  }, []);
  ...
  // logger.log(record.instances[0].call_number);
              ...
              <GridBlock title="" spacing={2}>
                ...
                </Grid2>
                {
                  <Grid2 size={2} className="record-thumbnail">
                    ...
                    <p>
                      {record.record_id && (
                        <img style={{ maxWidth: "100%" }} alt="" src={`/images/thumbnails/${record.record_id}.jpg`} />
                      )}
                    </p>
                  </Grid2>
                }
